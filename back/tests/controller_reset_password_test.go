	...
	for _, v := range samples {
		...
		if v.statusCode == 422 {
			...
			if responseMap["No_email"] != nil {
				assert.Equal(t, responseMap["No_email"], "Désolé, nous ne reconnaissons pas cet e-mail")
			}
	...
	for _, v := range samples {
		...
		if v.statusCode == 422 {
			...
			if responseMap["Invalid_token"] != nil {
				assert.Equal(t, responseMap["Invalid_token"], "Lien invalide. Essayez à nouveau.")
			}
			if responseMap["No_email"] != nil {
				assert.Equal(t, responseMap["No_email"], "Désolé, nous ne reconnaissons pas cet e-mail")
			}
			if responseMap["Invalid_Passwords"] != nil {
				assert.Equal(t, responseMap["Invalid_Passwords"], "Le mot de passe doit comporter au moins 6 caractères")
			}
			if responseMap["Empty_passwords"] != nil {
				assert.Equal(t, responseMap["Empty_passwords"], "Veuillez vous assurer que les deux champs sont remplis")
			}
			if responseMap["Password_unequal"] != nil {
				assert.Equal(t, responseMap["Password_unequal"], "Les mots de passe fournis ne correspondent pas")
			}
		}
	}
}
